...
def sex_race_count(request):
	...
	male = df['male'].tolist()
	female =df['female'].tolist()
	ln_male_count = len(male)
	ln_female_count =len(female)
...
def race_count(request):
	...
	race_counts =  ecrf.objects.filter(subject_race='Black or African American').count()
	print('race counts')
	print(race_counts)
	eth_list=list(eth)
...
def infa_count(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('influza_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['influza_testresult'] for ethn in eth_list)
	eth_dict=dict(ethnicity_dict)
	percent_eth = {k: round((v/y)*100) for k, v in eth_dict.items()} 
	print(percent_eth)#eth_dict={}
	#for i in eth_list:
	...
	color_num = len(pie)
	data = pd.Series(ethnicity_dict).reset_index(name='value').rename(columns={'index':'result'})
	data['angle'] = (data['value']/data['value'].sum()) * (2*pi)
	colours = viridis(len(eth_dict)) 
	print(colours)
	data['color'] = colours #chart_colors[:color_num]
	p = figure(
		plot_height=350,
		title="InfA Counts of Results",
		tooltips="@result: @value",x_range=(-0.5,1.0))
	p.wedge(x=0,y=1,radius=0.4,
		start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
		line_color = "white",color='color',legend='result', source=data)
	...
	context={'script11':script,'div11':div}
	return context

def infb_count(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('influzb_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['influzb_testresult'] for ethn in eth_list)
	eth_dict=dict(ethnicity_dict)
	percent_eth = {k: round((v/y)*100) for k, v in eth_dict.items()} 
	print(percent_eth)#eth_dict={}
	#for i in eth_list:
	...
	show(p)
	script, div=components(p)
	context={'script12':script,'div12':div}
	return context

def cov2_count(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('cov2_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['cov2_testresult'] for ethn in eth_list)
	eth_dict=dict(ethnicity_dict)
	percent_eth = {k: round((v/y)*100) for k, v in eth_dict.items()} 
	print(percent_eth)#eth_dict={}
	#for i in eth_list:
	...
	p = figure(
		plot_height=350,
		title="SARS-CoV-2 Counts of Results",
		tooltips="@result: @value",x_range=(-0.5,1.0))
	p.wedge(x=0,y=1,radius=0.4,
		start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
		line_color = "white",color='color',legend='result', source=data)
	...
	show(p)
	script, div=components(p)
	context={'script13':script,'div13':div}
	return context

def infa_percent(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('influza_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['influza_testresult'] for ethn in eth_list)
	...
	color_num = len(pie)
	data = pd.Series(percent_eth).reset_index(name='value').rename(columns={'index':'result'})
	data['angle'] = (data['value']/data['value'].sum()) * (2*pi)
	colours = viridis(len(eth_dict)) 
	print(colours)
	data['color'] = colours #chart_colors[:color_num]
	...
	show(p)
	script, div=components(p)
	context={'script14':script,'div14':div}
	return context

def infb_percent(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('influzb_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['influzb_testresult'] for ethn in eth_list)
	...
	show(p)
	script, div=components(p)
	context={'script15':script,'div15':div}
	return context

def cov2_percent(request):
	y = ecrf.objects.values('subjectid').count()
	eth = ecrf.objects.values('cov2_testresult')
	eth_list=list(eth)
	ethnicity_dict = Counter(ethn['cov2_testresult'] for ethn in eth_list)
	...
	p = figure(
		plot_height=350,
		title="SARS-CoV-2 Percent of Results",
		tooltips="@result: @value %",x_range=(-0.5,1.0))
	p.wedge(x=0,y=1,radius=0.4,
		start_angle=cumsum('angle', include_zero=True), end_angle=cumsum('angle'),
		line_color = "white",color='color',legend='result', source=data)
	...
	show(p)
	script, div=components(p)
	context={'script16':script,'div16':div}
	return context

def Merg_Dict(dict1,dict2,dict3,dict4,dict5,dict6,dict7,dict8,dict9,dict10,dict11,dict12,dict13,dict14,dict15,dict16):
	res ={**dict1,**dict2,**dict3,**dict4,**dict5,**dict6,**dict7,**dict8,**dict9,**dict10,**dict11,**dict12,**dict13,**dict14,**dict15,**dict16}
	return res
...
def metric_views(request):
	...
	infa_counts = infa_count(request)
	infb_counts = infb_count(request)
	infa_percents = infa_percent(request)
	infb_percents = infb_percent(request)
	cov2_counts = cov2_count(request)
	cov2_percents = cov2_percent(request)

	context = Merg_Dict(percent,ethn_count,ethn_percent,
				sex_eth_count,sex_eth_percent,symptom_counts,
				sex_r_percent,sex_r_count,r_count,r_percent,
				infa_counts,infb_counts,cov2_counts,cov2_percents,
				infa_percents,infb_percents) 
